...
/*
if (isset($_GET['new'])) {
    #Some shit to figure the new zpid
    #Already being done by calling page (list_zones) but may be a better way
}
*/
if (isset($_POST['zpid'])) {
    $zpid = $_POST['zpid'];
    if ($zp = $zpTable->GetZone($zpid)) {
        $i=0;
        ...
        foreach ($points as $row) {
            $pid = $row['pid'];
            $lat = $_POST["lat{$row['pid']}"];
            $long = $_POST["long{$row['pid']}"];
            if (strcmp($lat, $row['lat']) != 0) {
                echo "{$row['pid']} lat updated!<br>
                        {$row['lat']} to {$lat}<br>";
                 $updateList[$i] = array('pid' => (int)$row['pid'],
                                    'type' => (string)"ZPLat",
                                    'coord' => (string)$lat);
                 $i++;
            }
            if (strcmp($long, $row['long']) != 0) {
                echo "{$row['pid']} long updated!<br>";
                 $updateList[$i] = array('pid' => (int)$row['pid'],
                                    'type' => (string)"ZPLong",
                                    'coord' => (string)$long); # Need to pass updateList to the thing
                 $i++;
            }
        }
        if($zpTable->UpdateZone($zpid, $updateList)) echo "<h3>Updated!</h3>";
    }
    if(isset($_POST['npid'])) {
        $npid = $_POST['npid'];
        $nlat = $_POST['nlat'];
        $nlong = $_POST['nlong'];
        if($zpTable->NewZonePoint($zpid, $npid, $nlat, $nlong)) echo "<h3>Point Added!</h3>";
        else echo "<h3>Error Updating!</h3>";
    }

}

$zp = array('points' => array());
if (isset($_GET['new']) || $zp = $zpTable->GetZone($zpid) )  {

    $html = "<html>
    <form name =\"edit_zone\"
      action=\"edit_zone.php\" method=\"POST\">
        <input type=\"hidden\" name=\"zpid\" value=\"{$zpid}\">"; 
    
    $pid = 0;
    $points = $zp['points'];
    $html .= "<table border=\"1\">";
    $html .= "<tr><th>Point ID</th><th>Lat</th><th>Long</th></tr>";
    if (count($points) > 0) {
        foreach ($points as $row) {
            $pid = $row['pid'];
            $html .= "<tr>";
            $html .= "<td>{$pid}</td>";
            $html .= "<td><input type=\"text\" name=\"lat{$pid}\"
                        value=\"{$row['lat']}\"</td>";
            $html .= "<td><input type=\"text\" name=\"long{$pid}\"
                        value=\"{$row['long']}\"</td>";
            $html .= "</tr>";
        }
    }
    if (isset($_GET['new']) || $add > 0) {
        $pid++;
        $html .= "<tr>";
        $html .= "<td>{$pid}</td>";
        $html .= "<td><input type=\"text\" name=\"nlat\"
                    value=\"0\"</td>";
        $html .= "<td><input type=\"text\" name=\"nlong\"
                    value=\"0\"</td>";
        $html .= "</tr>
                    <input type=\"hidden\" name=\"npid\" value=\"{$pid}\">"; 
    }
    $html .= "</table>";
...
else echo "<html><h3>Invalid Zone Id</h3></html>"
?>
<?php
/*
require_once('../../config.inc.php');
